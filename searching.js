        ...
        if (this.queue.queue.length == 0) {
            ...
            for (let i=0;i<neighbors.length;i++) {
                ...
                if (n_node.color == COLORS["FILL"]) {
                    ...
                    this.queue.enqueue({node: n_node, priority: distance})
                    n_node.color = COLORS["SELECTED"]
                }
            }
            this.map.draw_nodes()
...
class A_star {
    ...
    start_search() {
        ...
        this.queue.enqueue({node: start_node, priority: distance, g_n: 0})
        this.parent[this.map.start.value] = null // set parent of start node as null
        this.interval = setInterval(this.search, get_speed())
        this.time = new Date()
    }
    ...
    search() {
        ...
        if (this.queue.queue.length == 0) {
            ...
            let node = this.queue.dequeue() // Pop first element from queue
            // Store g(n)
            let g_n = node.g_n
            node = node.node
            // Do not change color if node is start node
            ...
            for (let i=0;i<neighbors.length;i++) {
                ...
                if (n_node.color == COLORS["FILL"]) {
                    this.parent[n_node.value] = node.value
                    let distance = this.map.distance(n_node, this.map.goal)
                    // From parent to child manhattan distance will always be 1 in our MAP.
                    // Add 1 into g(n) of parent node to get g(n) of this child/neighbor node
                    let n_node_g_n = g_n+1
                    // Calculate f(n) = h(n) + g(n). And set it as priority
                    this.queue.enqueue({node: n_node, priority: distance + n_node_g_n, g_n: n_node_g_n})
                    n_node.color = COLORS["SELECTED"]
                }
            }
            this.map.draw_nodes()
